...
tuple<double, double, double, double, double, double,double> monte_carlo_put_price(const int& num_sims, const double& S, const double& K, const double& r, const double& v, const double& T, const double d) {
    double S_pv = S * exp(T*(r - 0.5*v*v));
    double S_cur = 0.0;
    double put_price_sum = 0.0;

    //this is for the greeks
    double S_pv_change_S_up = (S+d) * exp(T*(r - 0.5*v*v));
    double S_cur_change_S_up = 0.0;
    double put_price_S_up_sum = 0.0;
    double S_pv_change_S_down = (S-d) * exp(T*(r - 0.5*v*v));
    double S_cur_change_S_down = 0.0;
    double put_price_S_down_sum = 0.0;

    double S_pv_change_v_up = S * exp(T*(r - 0.5*(v+d)*(v+d)));
    double S_cur_change_v_up = 0.0;
    double put_price_v_up_sum = 0.0;
    double S_pv_change_v_down = S * exp(T*(r - 0.5*(v-d)*(v-d)));
    double S_cur_change_v_down = 0.0;
    double put_price_v_down_sum = 0.0;

    double S_pv_change_T_up = S * exp((T+d)*(r - 0.5*v*v));
    double S_cur_change_T_up = 0.0;
    double put_price_T_up_sum = 0.0;
    double S_pv_change_T_down = S * exp((T-d)*(r - 0.5*v*v));
    double S_cur_change_T_down = 0.0;
    double put_price_T_down_sum = 0.0;

    double S_pv_change_r_up = S * exp(T*((r+d) - 0.5*v*v));
    double S_cur_change_r_up = 0.0;
    double put_price_r_up_sum = 0.0;
    double S_pv_change_r_down = S * exp(T*((r-d) - 0.5*v*v));
    double S_cur_change_r_down = 0.0;
    double put_price_r_down_sum = 0.0;
    ...
    for (int i=0; i<num_sims; i++){
      ...
      put_price_S_up_sum += max(K - S_cur_change_S_up, 0.0);
      put_price_S_down_sum += max(K - S_cur_change_S_down, 0.0);

      put_price_v_up_sum += max(K - S_cur_change_v_up, 0.0);
      put_price_v_up_sum += max(K - S_cur_change_v_down, 0.0);

      put_price_T_up_sum += max(K - S_cur_change_T_up, 0.0);
      put_price_T_down_sum += max(K - S_cur_change_T_down, 0.0);

      put_price_r_up_sum += max(K - S_cur_change_r_up, 0.0);
      put_price_r_down_sum += max(K - S_cur_change_r_down, 0.0);

      

    };

    double put_price = (put_price_sum /num_sims)* exp(-r*T);

    double put_delta =  (put_price_S_up_sum - put_price_S_down_sum)/(2*d) * exp(-r*T) / num_sims;
    double put_gamma = (put_price_S_up_sum + put_price_S_down_sum - 2*put_price_sum)/(d*d) * exp(-r*T) / num_sims;
    double put_vega = sqrt((put_price_v_up_sum - put_price_v_down_sum)/(2*d*100) * d*exp(-r*T) / num_sims);

    // we have to individually calculate the rho and theta prices as exp(-r*T) will be different 

    double put_price_T_up = put_price_T_up_sum / num_sims * exp(-r*(T+d));
    double put_price_T_down = put_price_T_down_sum / num_sims * exp(-r*(T-d));

    double put_price_r_up = put_price_r_up_sum / num_sims * exp(-(r+d)*T);
    double put_price_r_down = put_price_r_down_sum / num_sims * exp(-(r-d)*T);

    double put_theta = (-put_price_T_up +  put_price_T_down)/(d*2*100);
    double put_rho = (put_price_r_up - put_price_r_down)/(d*2*100);

    return make_tuple(put_price, put_delta, put_gamma, put_vega, put_theta, put_rho, d);
